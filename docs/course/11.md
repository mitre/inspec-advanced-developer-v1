...
## 11. FilterTable exercise: Using FilterTable to write a Resource

### 11.1. When do I use FilterTable?
...
#### 11.1.1. May I have multiple FilterTable installations on a class?
...
#### 11.1.2. How do I declare my interaction with FilterTable? (Hands On)
...
#### 11.1.3. Run inspec shell with custom resource
...
#### 11.2.1. Some things are defined for you
...
#### 11.2.2. Custom Matcher Examples
- `has_car?
```ruby
...
#### 11.3.1. A `where` method you can call with hash params, with loose matching
...
#### 11.3.2. A `where` method you can call with a block, referencing some fields
...
#### 11.3.3. You can chain off of `where` or any other Table without re-fetching raw data
...
#### 11.3.4. An `entries` method that will return an array of Structs
...
#### 11.3.5. You get an `exist?` matcher defined on the resource and the table
...
#### 11.3.6. You get an `count` property defined on the resource and the table
...
#### 11.3.7. A `thing_ids` method that will return an array of plain values when called without params
...
#### 11.3.8. A `colors` method that will return a flattened and uniq'd array of values
...
#### 11.3.9. A `colors` method that can filter on a value and return a Table
...
#### 11.3.10. A `colors` method that can filter on a block and return a Table
...
#### 11.3.11. You can call `params` or `raw_data` on any Table to get the raw data
...
#### 11.3.12. You can call `resource_instance` on any Table to get the resource instance
