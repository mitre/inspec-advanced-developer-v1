...
## 12. FilterTable exercise: Using FilterTable to write a Resource

### 12.1. When do I use FilterTable?
...
#### 12.1.1. May I have multiple FilterTable installations on a class?
...
#### 12.1.2. How do I declare my interaction with FilterTable? (Hands On)
...
#### 12.1.3. Run inspec shell with custom resource
...
#### 12.2.1. Some things are defined for you
...
#### 12.2.2. Custom Matcher Examples
- `has_car?
```ruby
...
#### 12.3.1. A `where` method you can call with hash params, with loose matching
...
#### 12.3.2. A `where` method you can call with a block, referencing some fields
...
#### 12.3.3. You can chain off of `where` or any other Table without re-fetching raw data
...
#### 12.3.4. An `entries` method that will return an array of Structs
...
#### 12.3.5. You get an `exist?` matcher defined on the resource and the table
...
#### 12.3.6. You get an `count` property defined on the resource and the table
...
#### 12.3.7. A `thing_ids` method that will return an array of plain values when called without params
...
#### 12.3.8. A `colors` method that will return a flattened and uniq'd array of values
...
#### 12.3.9. A `colors` method that can filter on a value and return a Table
...
#### 12.3.10. A `colors` method that can filter on a block and return a Table
...
#### 12.3.11. You can call `params` or `raw_data` on any Table to get the raw data
...
#### 12.3.12. You can call `resource_instance` on any Table to get the resource instance
